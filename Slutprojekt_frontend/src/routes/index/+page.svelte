...
<main>
    ...
    <div class="relative w-[90%] h-auto md:w-[800px] md:h-[400px] mx-auto top-30 overflow-hidden rounded-xl mb-8 bg-gradient-to-t from-black/80 to-black/50">
        {#each topGames as game, index (game.id)}
          <div class="{index === currentSlide ? 'block' : 'hidden'} transition duration-500 ease-in-out">
            <div class="absolute top-30 md:top-0 right-5 p-4 text-white w-95">
                <h2 class="text-m md:text-xl font-bold">{game.name}</h2>
                <p class="text-xs md:text-sm line-clamp-2">{game.released}</p>
                <p class="text-xs md:text-sm text-gray-200 italic mt-2 leading-snug text-centre">{@html game.description}</p>
              </div>
            <img src={game.background_image} alt={game.name} class="relative w-65 h-30 mt-3 mb-45 md:mb-0 md:mt-10 left-20 md:left-0 md:w-100 md:h-75 object-cover rounded-xl" />
          </div>
        {/each}
...
<section class="grid grid-cols-1 md:grid-cols-1 gap-4">
    {#each games as game}
        <div class="relative ml-30 md:ml-80 bg-[#203454] w-80 lg:w-130 h-16 rounded-xl cursor-pointer"  on:click={() => handleGameClick(game)}>
...
<section class="hidden lg:block w-1/3 p-8 -mt-8">
  {#if selectedGame}
